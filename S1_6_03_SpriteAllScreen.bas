...
470 poke v + 41,2
480 rem step 8 -------------
490 rem animate along x and y coordinates all the screen
500 rem to use all the screen X axis that is 320 pixels long
510 rem we will put the sprite on the 256 position 
520 rem using register 16 for sprite 2 value is 4
530 rem 7=128 6=64 5=32 4=16 3=8 2=4 1=2 0=1
540 rem animate along  the y coordinate register 5
550 for y = 227 to 50 step -1
560 poke v + 5, y
570 next y
580 rem animate along the x set the x coordinate register 4
590 for x = 24 to 255
600 poke v + 4, x
610 next x
620 rem animate x from 256 to 320 for sprite 2
630 poke v + 16,4
640 for x = 0 to 63
650 poke v + 4, x
660 next x
670 rem poke v + 16,0
680 rem animate along  the y coordinate register 5
690 for y = 50 to 227
700 poke v + 5, y
710 next y
720 rem animate x from 320 to 256 for sprite 2
730 for x = 63 to 0 step -1
740 poke v + 4, x
750 next x
760 rem animate along the x set the x coordinate register 4
770 poke v + 16,0
780 for x = 255 to 24 step -1
790 poke v + 4, x
800 next x
810 goto 550
900 rem step 1 -------------
